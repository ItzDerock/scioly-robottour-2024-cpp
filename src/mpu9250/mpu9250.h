// Adapted from the Arduino MPU9250 library
...
public:
    ...
    bool setup(const uint8_t addr, const MPU9250Setting& mpu_setting = MPU9250Setting(), i2c_inst_t* w = i2c0) {
        // addr should be valid for MPU
        if ((addr < MPU9250_DEFAULT_ADDRESS) || (addr > MPU9250_DEFAULT_ADDRESS + 7)) {
            printf("I2C address 0x%x is not valid for MPU. Please check your I2C address.", addr);
            return false;
        }
        mpu_i2c_addr = addr;
        setting = mpu_setting;
        ...
        printf("querying mpu");
        bool ismpu = isConnectedMPU9250();
        printf("isConnectedMPU9250? %d", ismpu);
...
private:
    void initMPU9250() {
        ...
        sleep(100);                                  // Wait for all registers to reset
...
private:
    ...
    void set_acc_gyro_to_calibration() {
        ...
        sleep(40);                                  // accumulate 40 samples in 40 milliseconds = 480 bytes
    }
    ...
    void calibrate_mag_impl() {
        ...
        if (b_verbose) {
            printf("Mag Calibration done! (additional logging not implemented)\n");
        }
    ...
    void collect_mag_data_to(float* m_bias, float* m_scale) {
        ...
        for (uint16_t ii = 0; ii < sample_count; ii++) {
            ...
            if (MAG_MODE == 0x02) sleep(135);  // at 8 Hz ODR, new mag data is available every 125 ms
            if (MAG_MODE == 0x06) sleep(12);   // at 100 Hz ODR, new mag data is available every 10 ms
        }
    ...
    void write_byte(uint8_t address, uint8_t subAddress, uint8_t data) {
        ...
        int result = i2c_write_blocking(wire, address, buffer, 2, false);
        if (result != 2) {
          printf("write failed, %d != 2 (GENERIC ERROR: %d)\n", result, PICO_ERROR_GENERIC);
        }
  }
    ...
    uint8_t read_byte(uint8_t address, uint8_t subAddress) {
        ...
        i2c_write_blocking(wire, address, &subAddress, 1, true);
        int result = i2c_read_blocking(wire, address, &data, 1, false);

        if (result != 1) {
            printf("read failed, got %d != 1\n (GEN ERROR: %d)", result, PICO_ERROR_GENERIC); 
            return 0;
        }
    ...
    void read_bytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t* dest) {
        i2c_write_blocking(wire, address, &subAddress, 1, true);
        int result = i2c_read_blocking(wire, address, dest, count, false);    
        if (result != count) {
            printf("read failed, %d != %d", result, count);
        }
    }
